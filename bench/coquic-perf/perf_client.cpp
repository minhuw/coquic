...
std::optional<quic::QuicCoreTimePoint> QuicPerfClient::benchmark_next_wakeup() const {
    if (!timed_mode() || !benchmark_started_at_.has_value()) {
        return std::nullopt;
    }
    switch (phase_) {
    case BenchmarkPhase::warmup:
        return *benchmark_started_at_ + config_.warmup;
...
QuicPerfClient::next_wait_wakeup(std::optional<quic::QuicCoreTimePoint> core_next_wakeup) const {
    const auto benchmark_wakeup = benchmark_next_wakeup();
    if (!benchmark_wakeup.has_value()) {
        return core_next_wakeup;
    }
    if (!core_next_wakeup.has_value()) {
        return benchmark_wakeup;
    }
...
void QuicPerfClient::enter_measure_phase(quic::QuicCoreTimePoint now) {
    ...
    if (timed_bulk_mode()) {
        ...
        for (auto &[_, connection] : connections_) {
            (void)maybe_issue_crr_request(connection, now);
        }
    }
}
...
void QuicPerfClient::advance_benchmark_phase(quic::QuicCoreTimePoint now) {
    ...
    if (phase_ == BenchmarkPhase::warmup && now - *benchmark_started_at_ >= config_.warmup) {
        enter_measure_phase(now);
    }
    ...
    for (;;) {
        ...
        case io::QuicIoEvent::Kind::path_mtu_update:
            if (auto input = make_endpoint_input_from_io_event(*event); input.has_value()) {
                if (!handle_result(advance_endpoint(std::move(*input), event->now),
        ...
        case io::QuicIoEvent::Kind::rx_datagram:
            ...
            auto inbound_result = advance_endpoint(
...
bool QuicPerfClient::handle_result(quic::QuicCoreResult result, quic::QuicCoreTimePoint now) {
    ...
    if (result.send_sink_failed) {
        summary_.failure_reason = "client direct send sink failed";
        return false;
    }
    ...
    for (const auto &effect : result.effects) {
        ...
        if (const auto *state = std::get_if<quic::QuicCoreStateEvent>(&effect)) {
            if (state->change == quic::QuicCoreStateChange::failed) {
                ...
                summary_.failure_reason = describe_failed_connection(state->connection, now);
                return false;
            }
            if (state->change == quic::QuicCoreStateChange::handshake_ready) {
                ...
                auto command_result = advance_endpoint(
                    ...
                    now);
                if (command_result.local_error.has_value() || command_result.send_sink_failed) {
...
std::string QuicPerfClient::benchmark_phase_name() const {
    switch (phase_) {
    case BenchmarkPhase::warmup:
        return "warmup";
    case BenchmarkPhase::measure:
...
std::string QuicPerfClient::describe_failed_connection(quic::QuicConnectionHandle connection,
    ...
    description
        ...
        << " requests_completed=" << summary_.requests_completed;
    ...
    if (diagnostics_entry != diagnostics.end()) {
        ...
        if (!connection_diagnostics.streams.empty()) {
            ...
            description
                ...
                << " core_first_stream_sendable_bytes=" << stream.sendable_bytes
                << " core_first_stream_send_closed=" << static_cast<int>(stream.send_closed)
                << " core_first_stream_receive_closed=" << static_cast<int>(stream.receive_closed)
                << " core_first_stream_send_fin_state=" << static_cast<int>(stream.send_fin_state)
                << " core_first_stream_send_flow_limit=" << stream.send_flow_control_limit
                << " core_first_stream_receive_flow_limit=" << stream.receive_flow_control_limit
                << " core_first_stream_highest_received=" << stream.highest_received_offset
...
bool QuicPerfClient::maybe_close_rr_connection(ConnectionState &connection,
                                               quic::QuicCoreTimePoint now) {
    const bool force_timed_drain_close = timed_rr_mode() && phase_ == BenchmarkPhase::drain;
    if (connection.close_requested ||
...
bool QuicPerfClient::maybe_close_crr_connection(ConnectionState &connection,
                                                quic::QuicCoreTimePoint now) {
    const bool force_timed_drain_close = timed_crr_mode() && phase_ == BenchmarkPhase::drain;
    if (connection.close_requested ||
        (!force_timed_drain_close && !connection.outstanding_requests.empty())) {
        return true;
    }

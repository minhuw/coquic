...
function isRunActive(runToken) {
  return runToken === activeRunToken && demoState !== "idle";
}
...
function clearPackets() {
  ...
  el("packet-rail").querySelectorAll(".packet").forEach((packet) => packet.remove());
  el("packet-rail").querySelectorAll(".datagram").forEach((datagram) => datagram.remove());
}
...
el("stop").addEventListener("click", pauseDemo);

el("step").addEventListener("click", requestNextStep);

el("download-pcap").addEventListener("click", downloadPcap);

el("packet-modal-close").addEventListener("click", closePacketModal);

el("packet-modal").addEventListener("click", (event) => {

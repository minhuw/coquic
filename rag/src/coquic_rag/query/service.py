...
from coquic_rag.embed.provider import EmbeddingProvider, OpenRouterEmbedder
...
def _configured_qdrant_api_key(paths: ProjectPaths) -> str | None:
    return paths.qdrant_api_key or os.getenv("COQUIC_QDRANT_API_KEY")
...
def get_index_status(
    ...
    *,
    ...
    store = QdrantSectionStore(
        ...
        qdrant_api_key=_configured_qdrant_api_key(resolved_paths),
...
class QueryService:
    def __init__(
        ...
        *,
        ...
        self._require_artifacts_for_search = require_artifacts_for_search
        self._store = QdrantSectionStore(
            state_dir=self._paths.qdrant_dir,
            qdrant_url=qdrant_url,
            qdrant_api_key=_configured_qdrant_api_key(self._paths),
            collection_name=collection_name,
        )
        self._loaded = False
        self._optional_artifacts_checked = False
    ...
    def search_sections(
        ...
        *,
        ...
        if self._require_artifacts_for_search:
            self._ensure_loaded()
        else:
            self._load_optional_artifacts()
        resolved_doc_id = _normalize_doc_selector(doc_id, rfc=rfc)
    ...
    def _load_optional_artifacts(self) -> None:
        if self._loaded or self._optional_artifacts_checked:
            return
        self._optional_artifacts_checked = True
        if not all(path.is_file() for path in _required_artifact_paths(self._paths)):
            return
        self._load_artifacts()

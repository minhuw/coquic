...
def test_qdrant_store_passes_api_key_for_remote_backend(
    tmp_path: Path,
    monkeypatch,
) -> None:
    calls: list[dict[str, Any]] = []
...
def test_qdrant_store_upserts_sections_and_filters_search_results(
    ...
    assert [(summary.doc_id, summary.sections) for summary in summaries] == [
        ("draft-ietf-quic-qlog-main-schema-13", 1),
        ("rfc9000", 1),
    ]

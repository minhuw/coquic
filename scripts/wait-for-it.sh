...
echoerr() {
    if [[ ${WAITFORIT_QUIET:-0} -ne 1 ]]; then
        echo "$@" 1>&2
    fi
}
...
Usage:
    ...
    -p PORT | --port=PORT       TCP port under test
                                Alternatively, specify the host and port as
                                host:port.
...
wait_for() {
    ...
    while :; do
        if [[ ${WAITFORIT_ISBUSY:-0} -eq 1 ]]; then
...
wait_for_wrapper() {
    ...
    if [[ ${WAITFORIT_QUIET:-0} -eq 1 ]]; then
        timeout "$WAITFORIT_BUSYTIMEFLAG" "$WAITFORIT_TIMEOUT" \
            "$0" --quiet --child --host="$WAITFORIT_HOST" --port="$WAITFORIT_PORT" \

...
function mapError(error: unknown): Problem {
  if (error instanceof InvalidRequestError) {
    return { code: "INVALID_REQUEST", status: 400, message: "The cloud task detail request is invalid.", retryable: false };
  }
...
export async function GET(_request: Request, context: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    const data = await getCloudRepository().getTaskDetail(taskId);
    if (!data) {

...
export function LazyTranscript({ taskId, runId, path, initial, initialCursor = null, initialHasMore, fileRevision }: Props) {
  ...
  useEffect(() => {
    ...
    if (!loaded.current) { setCursor(initialCursor); setHasMore(initialHasMore ?? Boolean(runId && path)); }
  ...
  async function loadMore() {
    ...
    try {
      ...
      setRecords((current) => { const seen = new Set(current.map((record) => record.ordinal)); return [...current, ...payload.data.records.filter((record) => !seen.has(record.ordinal))].sort((left, right) => left.ordinal - right.ordinal); });

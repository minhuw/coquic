...
```bash
nix develop -c npm --prefix site/next ci
nix develop -c npm --prefix site/next run build:demo
```

Package the standalone Next.js server bundle:

```bash
nix develop -c npm --prefix site/next run package:demo
```
...
nix develop -c npm --prefix site/next run dev
```
...
```bash
nix develop -c npm --prefix site/next run build:wasm
nix develop -c npm --prefix site/next run dev

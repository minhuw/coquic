...
COQUIC_NO_PROFILE bool core_ffi_conversion_coverage_for_tests() {
    ...
    record(coquic_ffi_abi_version() == COQUIC_FFI_ABI_VERSION);
    ...
    record(to_string("abc", 0).empty());
    record(to_string("abc", 3) == "abc");
    record(to_vector(coquic_bytes_t{.data = nullptr, .length = 3}).empty());
    record(to_vector(coquic_bytes_t{.data = bytes.data(), .length = 0}).empty());
    record(to_vector(coquic_bytes_t{.data = bytes.data(), .length = bytes.size()}).size() ==
           bytes.size());
    record(!to_optional(coquic_optional_route_handle_t{.has_value = 0, .value = 7}).has_value());
    record(to_optional(coquic_optional_route_handle_t{.has_value = 1, .value = 7}) ==
           std::optional<coquic::core::RouteHandle>{7});
    record(from_optional_route_handle(std::nullopt).has_value == 0);
    record(from_optional_route_handle(std::optional<coquic::core::RouteHandle>{8}).value == 8);
    record(from_optional_connection_handle(std::nullopt).has_value == 0);
    record(
        from_optional_connection_handle(std::optional<coquic::core::ConnectionHandle>{9}).value ==
        9);
    record(from_optional_stream_id(std::nullopt).has_value == 0);
    record(from_optional_stream_id(std::optional<coquic::core::StreamId>{10}).value == 10);
    record(!from_optional(std::nullopt).has_value);
    record(from_optional(TimePoint{std::chrono::microseconds{11}}).value == 11);
    record(role_to_cpp(COQUIC_ROLE_SERVER) == coquic::core::Role::server);
    record(role_to_cpp(static_cast<coquic_role_t>(99)) == coquic::core::Role::client);
    record(congestion_control_to_cpp(COQUIC_CONGESTION_CONTROL_CUBIC) ==
           coquic::core::CongestionControl::cubic);
    record(congestion_control_to_cpp(COQUIC_CONGESTION_CONTROL_BBR) ==
           coquic::core::CongestionControl::bbr);
    record(congestion_control_to_cpp(COQUIC_CONGESTION_CONTROL_COPA) ==
           coquic::core::CongestionControl::copa);
    record(congestion_control_to_cpp(static_cast<coquic_congestion_control_t>(99)) ==
           coquic::core::CongestionControl::newreno);
    record(ecn_to_cpp(COQUIC_ECN_NOT_ECT) == coquic::core::EcnCodepoint::not_ect);
    record(ecn_to_cpp(COQUIC_ECN_ECT0) == coquic::core::EcnCodepoint::ect0);
    record(ecn_to_cpp(COQUIC_ECN_ECT1) == coquic::core::EcnCodepoint::ect1);
    record(ecn_to_cpp(COQUIC_ECN_CE) == coquic::core::EcnCodepoint::ce);
    record(ecn_to_cpp(static_cast<coquic_ecn_codepoint_t>(99)) ==
           coquic::core::EcnCodepoint::unavailable);
    record(from_cpp(coquic::core::EcnCodepoint::not_ect) == COQUIC_ECN_NOT_ECT);
    record(from_cpp(coquic::core::EcnCodepoint::ect0) == COQUIC_ECN_ECT0);
    record(from_cpp(coquic::core::EcnCodepoint::ect1) == COQUIC_ECN_ECT1);
    record(from_cpp(coquic::core::EcnCodepoint::ce) == COQUIC_ECN_CE);
    record(migration_reason_to_cpp(COQUIC_MIGRATION_REASON_PREFERRED_ADDRESS) ==
           coquic::core::MigrationReason::preferred_address);
    record(migration_reason_to_cpp(static_cast<coquic_migration_reason_t>(99)) ==
           coquic::core::MigrationReason::active);
    record(from_cpp(coquic::core::StateChange::handshake_confirmed) ==
           COQUIC_STATE_CHANGE_HANDSHAKE_CONFIRMED);
    record(from_cpp(coquic::core::StateChange::failed) == COQUIC_STATE_CHANGE_FAILED);
    record(from_cpp(coquic::core::Lifecycle::accepted) == COQUIC_LIFECYCLE_ACCEPTED);
    record(from_cpp(coquic::core::Lifecycle::closed) == COQUIC_LIFECYCLE_CLOSED);
    record(from_cpp(coquic::core::ZeroRttStatus::not_attempted) == COQUIC_ZERO_RTT_NOT_ATTEMPTED);
    record(from_cpp(coquic::core::ZeroRttStatus::attempted) == COQUIC_ZERO_RTT_ATTEMPTED);
    record(from_cpp(coquic::core::ZeroRttStatus::accepted) == COQUIC_ZERO_RTT_ACCEPTED);
    record(from_cpp(coquic::core::ZeroRttStatus::rejected) == COQUIC_ZERO_RTT_REJECTED);
    record(from_cpp(coquic::core::PacketInspectionDirection::inbound) ==
           COQUIC_PACKET_INSPECTION_INBOUND);
    record(from_cpp(coquic::core::PacketInspectionPacketType::zero_rtt) ==
           COQUIC_PACKET_INSPECTION_ZERO_RTT);
    record(from_cpp(coquic::core::PacketInspectionPacketType::handshake) ==
           COQUIC_PACKET_INSPECTION_HANDSHAKE);
    record(from_cpp(coquic::core::PacketInspectionPacketType::one_rtt) ==
           COQUIC_PACKET_INSPECTION_ONE_RTT);
    ...
    record(from_cpp(coquic::core::LocalErrorCode::invalid_stream_id) ==
           COQUIC_LOCAL_ERROR_INVALID_STREAM_ID);
    record(from_cpp(coquic::core::LocalErrorCode::invalid_stream_direction) ==
           COQUIC_LOCAL_ERROR_INVALID_STREAM_DIRECTION);
    record(from_cpp(coquic::core::LocalErrorCode::send_side_closed) ==
           COQUIC_LOCAL_ERROR_SEND_SIDE_CLOSED);
    record(from_cpp(coquic::core::LocalErrorCode::receive_side_closed) ==
           COQUIC_LOCAL_ERROR_RECEIVE_SIDE_CLOSED);
    record(from_cpp(coquic::core::LocalErrorCode::final_size_conflict) ==
           COQUIC_LOCAL_ERROR_FINAL_SIZE_CONFLICT);
    record(from_cpp(coquic::core::LocalErrorCode::datagram_not_supported) ==
           COQUIC_LOCAL_ERROR_DATAGRAM_NOT_SUPPORTED);
    record(from_cpp(coquic::core::LocalErrorCode::datagram_too_large) ==
           COQUIC_LOCAL_ERROR_DATAGRAM_TOO_LARGE);
           ...
           COQUIC_LOCAL_ERROR_UNSUPPORTED_OPERATION);
    {
        ...
        const std::array<std::uint32_t, 2> versions{1, 0x6b3343cfu};
        config.role = COQUIC_ROLE_SERVER;
        config.supported_versions = versions.data();
        config.supported_versions_count = versions.size();
        config.verify_peer = 1;
        config.retry_enabled = 1;
        config.application_protocol = "ffi";
        config.application_protocol_length = 3;
        coquic_tls_identity_t identity{
            .certificate_pem = "cert",
            .certificate_pem_length = 4,
            .private_key_pem = "key",
            .private_key_pem_length = 3,
        };
        config.identity = &identity;
        config.zero_rtt.application_context =
            coquic_bytes_t{.data = bytes.data(), .length = bytes.size()};
        const auto converted = to_cpp(config);
        record(converted.role == coquic::core::Role::server);
        record(converted.supported_versions.size() == versions.size());
        record(converted.identity.has_value());
        record(converted.zero_rtt.application_context.size() == bytes.size());
    }
    {
        coquic_client_connection_config_t config{};
        coquic_client_connection_config_init(&config);
        config.source_connection_id = coquic_bytes_t{.data = bytes.data(), .length = bytes.size()};
        config.initial_destination_connection_id =
            coquic_bytes_t{.data = bytes.data(), .length = bytes.size()};
        config.original_destination_connection_id =
            coquic_bytes_t{.data = bytes.data(), .length = bytes.size()};
        config.has_original_destination_connection_id = 1;
        config.retry_source_connection_id =
            coquic_bytes_t{.data = bytes.data(), .length = bytes.size()};
        config.has_retry_source_connection_id = 1;
        config.retry_token = coquic_bytes_t{.data = bytes.data(), .length = bytes.size()};
        config.reacted_to_version_negotiation = 1;
        coquic_resumption_state_t resumption{
            .serialized = coquic_bytes_t{.data = bytes.data(), .length = bytes.size()},
        };
        config.resumption_state = &resumption;
        const auto converted = to_cpp(config);
        record(converted.original_destination_connection_id.has_value());
        record(converted.retry_source_connection_id.has_value());
        record(converted.resumption_state.has_value());
        record(converted.reacted_to_version_negotiation);
    }
    {
        const auto make_sized_input = [](coquic_connection_input_kind_t kind, std::size_t size) {
            coquic_connection_input_t input{};
            input.kind = kind;
            switch (kind) {
            case COQUIC_CONNECTION_INPUT_SEND_STREAM:
                input.as.send_stream.size = size;
                input.as.send_stream.stream_id = 1;
                break;
            case COQUIC_CONNECTION_INPUT_SEND_DATAGRAM:
                input.as.send_datagram.size = size;
                break;
            case COQUIC_CONNECTION_INPUT_RESET_STREAM:
                input.as.reset_stream.size = size;
                input.as.reset_stream.stream_id = 2;
                break;
            case COQUIC_CONNECTION_INPUT_STOP_SENDING:
                input.as.stop_sending.size = size;
                input.as.stop_sending.stream_id = 3;
                break;
            case COQUIC_CONNECTION_INPUT_CLOSE:
                input.as.close.size = size;
                input.as.close.reason_phrase = "bye";
                input.as.close.reason_phrase_length = 3;
                break;
            case COQUIC_CONNECTION_INPUT_REQUEST_MIGRATION:
                input.as.request_migration.size = size;
                input.as.request_migration.route_handle = 4;
                input.as.request_migration.reason = COQUIC_MIGRATION_REASON_PREFERRED_ADDRESS;
                break;
            default:
                break;
            }
        ...
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_SEND_STREAM, 0)) == std::nullopt);
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_SEND_STREAM, kSendStreamDataSizeV1))
                   .has_value());
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_SEND_DATAGRAM, 0)) == std::nullopt);
        record(
            to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_SEND_DATAGRAM, kSendDatagramDataSizeV1))
                .has_value());
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_RESET_STREAM, 0)) == std::nullopt);
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_RESET_STREAM, kResetStreamSizeV1))
                   .has_value());
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_STOP_SENDING, 0)) == std::nullopt);
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_STOP_SENDING, kStopSendingSizeV1))
                   .has_value());
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_CLOSE, 0)) == std::nullopt);
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_CLOSE, kCloseConnectionSizeV1))
                   .has_value());
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_REQUEST_KEY_UPDATE, 0)).has_value());
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_REQUEST_MIGRATION, 0)) ==
               std::nullopt);
        record(to_cpp(make_sized_input(COQUIC_CONNECTION_INPUT_REQUEST_MIGRATION,
                                       kRequestConnectionMigrationSizeV1))
                   .has_value());
        record(to_cpp(make_sized_input(static_cast<coquic_connection_input_kind_t>(99), 0)) ==
               std::nullopt);
    }
    ...
    result.effects = {
        ...
        coquic::core::PeerPreferredAddressAvailable{
            .connection = 6,
            .preferred_address =
                coquic::core::PreferredAddress{
                    .connection_id = {std::byte{0x06}},
                    .stateless_reset_token = {std::byte{0x07}},
                },
        },
        coquic::core::ResumptionStateAvailable{
            .connection = 7,
            .state =
                coquic::core::ResumptionState{
                    .serialized = {std::byte{0x08}},
                },
        },
        ...
        coquic::core::PacketInspection{
            ...
            .packet_length = 13,
            .version = 1,
            .destination_connection_id = {std::byte{0x09}},
            .source_connection_id = {std::byte{0x0a}},
            .token = {std::byte{0x0b}},
            .spin_bit = true,
            .key_phase = true,
            .packet_number_length = 2,
            .packet_number = 14,
            .encrypted_packet = {std::byte{0x0c}},
            .plaintext_payload = {std::byte{0x0d}},
        },
    ...
    coquic_result result_storage(result);
    record(coquic_result_effect_count(nullptr) == 0);
    record(coquic_result_effect_count(&result_storage) == result.effects.size());
    coquic_effect_t effect{};
    record(coquic_result_effect_at(nullptr, 0, &effect) == COQUIC_STATUS_INVALID_ARGUMENT);
    record(coquic_result_effect_at(&result_storage, 0, nullptr) == COQUIC_STATUS_INVALID_ARGUMENT);
    record(coquic_result_effect_at(&result_storage, result.effects.size(), &effect) ==
           COQUIC_STATUS_INVALID_ARGUMENT);
    record(coquic_result_effect_at(&result_storage, 0, &effect) == COQUIC_STATUS_OK);
    record(effect.kind == COQUIC_EFFECT_SEND_DATAGRAM);
    record(coquic_result_next_wakeup(nullptr).has_value == 0);
    result_storage.result.next_wakeup = TimePoint{std::chrono::microseconds{123}};
    record(coquic_result_next_wakeup(&result_storage).value == 123);
    record(coquic_result_has_local_error(nullptr) == 0);
    coquic_local_error_t local_error{};
    record(coquic_result_local_error(nullptr, &local_error) == COQUIC_STATUS_INVALID_ARGUMENT);
    record(coquic_result_local_error(&result_storage, nullptr) == COQUIC_STATUS_INVALID_ARGUMENT);
    record(coquic_result_local_error(&result_storage, &local_error) ==
           COQUIC_STATUS_INVALID_ARGUMENT);
    result_storage.result.local_error =
        coquic::core::LocalError{.connection = 11,
                                 .code = coquic::core::LocalErrorCode::datagram_too_large,
                                 .stream_id = 12};
    record(coquic_result_has_local_error(&result_storage) == 1);
    record(coquic_result_local_error(&result_storage, &local_error) == COQUIC_STATUS_OK);
    record(local_error.stream_id.has_value == 1);
    record(coquic_result_send_continuation_pending(nullptr) == 0);
    result_storage.result.send_continuation_pending = true;
    record(coquic_result_send_continuation_pending(&result_storage) == 1);
    record(coquic_endpoint_connection_count(nullptr) == 0);
    record(coquic_endpoint_has_send_continuation_pending(nullptr) == 0);
    record(coquic_endpoint_has_pending_stream_send(nullptr) == 0);
    record(coquic_endpoint_next_wakeup(nullptr).has_value == 0);
    record(coquic_endpoint_create(nullptr, nullptr) == COQUIC_STATUS_INVALID_ARGUMENT);
    coquic_endpoint_t *endpoint = nullptr;
    coquic_endpoint_config_t invalid_endpoint_config{};
    record(coquic_endpoint_create(&invalid_endpoint_config, &endpoint) ==
           COQUIC_STATUS_INVALID_ARGUMENT);
    record(endpoint == nullptr);
    coquic_result_t *out_result = reinterpret_cast<coquic_result_t *>(0x1);
    coquic_connection_input_t invalid_advance{};
    record(coquic_connection_advance(nullptr, 1, nullptr, 0, &out_result) ==
           COQUIC_STATUS_INVALID_ARGUMENT);
    record(out_result == nullptr);
    out_result = reinterpret_cast<coquic_result_t *>(0x1);
    record(coquic_connection_advance(nullptr, 1, &invalid_advance, 0, &out_result) ==
           COQUIC_STATUS_INVALID_ARGUMENT);
    record(out_result == nullptr);
    coquic_connection_handle_t out_connection = 99;
    record(coquic_quic_connect(nullptr, nullptr, 0, &out_connection, &out_result) ==
           COQUIC_STATUS_INVALID_ARGUMENT);
    record(out_connection == 0);

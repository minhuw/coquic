...
bool runtime_parser_routing_coverage_check(bool &ok, std::string_view label, bool condition) {
    if (!condition) {
        std::cerr << "http09 runtime parser/routing coverage failed: " << label << '\n';
        ok = false;
    }
    return condition;
}
...
bool runtime_parser_and_utility_coverage_for_tests() {
    ...
    runtime_parser_routing_coverage_check(ok, "parse_io_backend_kind accepts socket",
                                          parse_io_backend_kind("socket") ==
                                              io::QuicIoBackendKind::socket);
    ...
    runtime_parser_routing_coverage_check(
        ok, "format_sockaddr_for_trace prints numeric host and port",
...

    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "open_and_bind_udp_socket succeeds for loopback ephemeral binds", socket_fd >= 0);
        if (socket_fd >= 0) {
            ::close(socket_fd);
        }
    }

    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "open_and_bind_udp_socket reports socket creation failures",
...

    {
        ...
        runtime_parser_routing_coverage_check(ok, "open_and_bind_udp_socket reports bind failures",
                                              open_and_bind_udp_socket(bind_address, "client") < 0);
    }

    runtime_parser_routing_coverage_check(ok, "v1 initial header type recognized",
                                          is_initial_long_header_type(kQuicVersion1, 0x00u));
    runtime_parser_routing_coverage_check(ok, "v2 initial header type recognized",
                                          is_initial_long_header_type(kQuicVersion2, 0x01u));
    runtime_parser_routing_coverage_check(ok, "big-endian u32 reader decodes version words",
                                          read_u32_be_at(
                                              std::array{
                                                  std::byte{0x01},
                                                  std::byte{0x23},
    ...
    runtime_parser_routing_coverage_check(
        ok, "short header without fixed bit is rejected",
        !parse_server_datagram_for_routing(std::vector<std::byte>{std::byte{0x00}}).has_value());
    runtime_parser_routing_coverage_check(
        ok, "short header shorter than runtime cid length is rejected",
             ...
             .has_value());
    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "short header kind is reported",
            parsed.has_value() && parsed->kind == ParsedServerDatagram::Kind::short_header);
    }

    runtime_parser_routing_coverage_check(
        ok, "long header missing fixed bit is rejected",
        !parse_server_datagram_for_routing(std::vector<std::byte>{std::byte{0x80}, std::byte{0x00},
                                                                  std::byte{0x00}, std::byte{0x00},
                                                                  std::byte{0x01}, std::byte{0x00},
                                                                  std::byte{0x00}})
             .has_value());
    runtime_parser_routing_coverage_check(
        ok, "fixed-bit long headers shorter than the minimum prefix are rejected",
        !parse_server_datagram_for_routing(std::vector<std::byte>{std::byte{0xc0}, std::byte{0x00},
                                                                  std::byte{0x00}, std::byte{0x00},
                                                                  std::byte{0x01}, std::byte{0x00}})
             .has_value());
    runtime_parser_routing_coverage_check(
        ...
        !parse_server_datagram_for_routing(
             ...
             .has_value());
    runtime_parser_routing_coverage_check(
        ok, "destination connection id length overflow is rejected",
        !parse_server_datagram_for_routing(std::vector<std::byte>{std::byte{0xc0}, std::byte{0x00},
                                                                  std::byte{0x00}, std::byte{0x00},
                                                                  std::byte{0x01}, std::byte{0x08},
                                                                  std::byte{0x01}})
             .has_value());
    runtime_parser_routing_coverage_check(
        ok, "source connection id length overflow is rejected",
        !parse_server_datagram_for_routing(
...

    {
        ...
        runtime_parser_routing_coverage_check(ok, "unsupported versions parse as probes",
                                              parsed.has_value());
        runtime_parser_routing_coverage_check(
            ok, "unsupported versions are classified as unsupported long headers",
            parsed.has_value() &&
...

    {
        ...
        runtime_parser_routing_coverage_check(ok, "supported non-initial long headers parse",
                                              parsed.has_value());
        runtime_parser_routing_coverage_check(
            ok, "supported non-initial long headers keep empty retry tokens",
...

    {
        ...
        runtime_parser_routing_coverage_check(ok, "supported initial long headers parse",
                                              parsed.has_value());
        runtime_parser_routing_coverage_check(
            ok, "supported initial long headers preserve retry tokens",
...

    {
        ...
        runtime_parser_routing_coverage_check(ok, "v2 initial long headers parse",
                                              parsed.has_value());
        runtime_parser_routing_coverage_check(
            ok, "v2 initial long headers use the v2 initial type mapping",
            parsed.has_value() && parsed->kind == ParsedServerDatagram::Kind::supported_initial);
    }
...
bool runtime_retry_and_probe_coverage_for_tests() {
    ...
    const auto retry_destination_connection_id = make_runtime_connection_id(std::byte{0x51}, 1);
    const auto retry_source_connection_id = make_runtime_connection_id(std::byte{0x61}, 2);
    const ParsedServerDatagram supported_initial{
        .kind = ParsedServerDatagram::Kind::supported_initial,
        .version = kQuicVersion1,
        .destination_connection_id = retry_destination_connection_id,
    ...
    runtime_parser_routing_coverage_check(
        ok, "retry token encodes the runtime prefix",
    ...
    PendingRetryToken pending_retry{
        .original_destination_connection_id = retry_destination_connection_id,
    ...
    runtime_parser_routing_coverage_check(
        ok, "retry peer matching accepts the original peer",
...

    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "retry lookup rejects peer mismatches",
            !lookup_retry_context(parsed, other_peer, sizeof(sockaddr_in), retry_tokens)
                 .has_value());
        runtime_parser_routing_coverage_check(
            ok, "retry lookup keeps stored tokens on mismatch",
...

    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "retry lookup rejects destination connection id mismatches",
...

    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "retry lookup rejects version mismatches",
            !lookup_retry_context(parsed, peer, sizeof(sockaddr_in), retry_tokens).has_value());
    }

    {
        ...
        runtime_parser_routing_coverage_check(ok, "retry lookup accepts matching retry contexts",
                                              context.has_value());
        runtime_parser_routing_coverage_check(ok, "successful retry lookup erases consumed tokens",
                                              retry_tokens.empty());
    }

    {
        ...
        RetryTokenStore retry_tokens;
        runtime_parser_routing_coverage_check(
            ok, "retry send rejects missing source connection ids",
...

    {
        ...
        runtime_parser_routing_coverage_check(ok, "retry send emits a retry datagram",
                                              send_retry_for_initial(/*fd=*/21, supported_initial,
                                                                     peer, sizeof(sockaddr_in),
                                                                     retry_tokens,
                                                                     /*connection_index=*/5));
        runtime_parser_routing_coverage_check(ok, "retry send records exactly one sendto call",
                                              g_recorded_sendto_for_tests.calls == 1);
        runtime_parser_routing_coverage_check(ok, "retry send stores one pending retry token",
                                              retry_tokens.size() == 1);
    }

    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "retry helper returns nullopt when retry is disabled",
        ...
        runtime_parser_routing_coverage_check(
            ok, "retry helper returns nullopt when the client already presented a token",
            !maybe_send_retry_for_supported_initial(
...

    {
        ...
        const auto retry_result = maybe_send_retry_for_supported_initial(
            ...
            retry_tokens, next_connection_index);
        runtime_parser_routing_coverage_check(
            ok, "retry helper returns an immediate result for tokenless initials",
            retry_result.has_value() && retry_result.value());
        runtime_parser_routing_coverage_check(
            ok, "retry helper increments the next connection index", next_connection_index == 13);
    }

    {
        ...
        RetryTokenStore retry_tokens;
        runtime_parser_routing_coverage_check(
            ok, "populate retry context succeeds when retry is disabled",
        ...
        runtime_parser_routing_coverage_check(
            ok, "populate retry context leaves retry context empty when disabled",
...

    {
        ...
        parsed.destination_connection_id = pending_retry.retry_source_connection_id;
        runtime_parser_routing_coverage_check(
            ok, "populate retry context returns the matched context",
            populate_retry_context_if_required(/*retry_enabled=*/true, parsed, peer,
                                               sizeof(sockaddr_in), retry_tokens, retry_context));
        runtime_parser_routing_coverage_check(
            ok, "populate retry context fills the optional context", retry_context.has_value());
    }

    {
        ...
        parsed.destination_connection_id = pending_retry.retry_source_connection_id;
        runtime_parser_routing_coverage_check(
            ok, "populate retry context rejects invalid retry tokens",
            !populate_retry_context_if_required(/*retry_enabled=*/true, parsed, peer,
                                                sizeof(sockaddr_in), retry_tokens, retry_context));
    }

    {
        ...
        const auto preparation = prepare_supported_initial_retry_handling(
            ...
            retry_tokens, next_connection_index);
        runtime_parser_routing_coverage_check(
            ok, "retry preparation returns an immediate result when it emits a retry",
...

    {
        ...
        const auto preparation = prepare_supported_initial_retry_handling(
            ...
            retry_tokens, next_connection_index);
        runtime_parser_routing_coverage_check(
            ok, "retry preparation returns a retry context when the token matches",
...

    {
        ...
        const auto preparation = prepare_supported_initial_retry_handling(
            ...
            retry_tokens, next_connection_index);
        runtime_parser_routing_coverage_check(
            ok, "retry preparation turns invalid retry tokens into immediate no-op results",
            preparation.immediate_result.has_value() && preparation.immediate_result.value());
    }

    runtime_parser_routing_coverage_check(
        ok, "version negotiation probes under the minimum size are ignored",
...

    {
        ...
        missing_source.source_connection_id.reset();
        runtime_parser_routing_coverage_check(
            ok, "version negotiation send rejects missing source connection ids",
            !send_version_negotiation_for_probe(
                /*fd=*/37,
...

    {
        ...
        runtime_parser_routing_coverage_check(
            ok, "version negotiation sends a probe response for oversized unsupported initials",
            send_version_negotiation_for_probe(
                /*fd=*/38,
                std::vector<std::byte>(kMinimumClientInitialDatagramBytes, std::byte{0x00}),
                supported_initial, peer, sizeof(sockaddr_in)));
        runtime_parser_routing_coverage_check(ok, "version negotiation send records a datagram",
                                              g_recorded_sendto_for_tests.calls == 1);
    }

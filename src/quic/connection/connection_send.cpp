  ...
  public:
    ...
    explicit ApplicationCandidateFrameBuilder(const Args &args)
        : scratch_(args.scratch), alternate_scratch_(args.alternate_scratch),
          ...
          application_close_frame_(args.application_close_frame), include_ping_(args.include_ping) {
    }
    ...
    std::span<const Frame> current() {
        return cache_.current(scratch_, received_packets_, crypto_frames_, include_handshake_done_,
                              ...
                              application_close_frame_, include_ping_);
    }
...
QuicConnection::drain_fast_bulk_stream_datagrams(QuicCoreTimePoint now, bool continue_paced_burst,
    ...
    auto stream_wire_budget = application_stream_frame_budget(
    ...
    while (emitted < max_datagrams) {
        ...
        const auto visit_range = [&](auto begin, auto end) {
            for (auto it = begin; it != end; ++it) {
                auto &candidate_stream = it->second;
                if (candidate_stream.reset_state == StreamControlFrameState::none &&
                    candidate_stream.sendable_bytes() != 0 &&
                    !candidate_stream.send_buffer.has_lost_data() &&
                    !stream_fin_sendable(candidate_stream)) {
                    selected = it;
                    return false;
                }
            }
        ...
        fragments.clear();
        auto &selected_stream = selected->second;
        ...
        selected_stream.append_send_fragments(
            StreamSendBudget{
                .packet_bytes = static_cast<std::size_t>(fast_stream_packet_payload_budget),
                .new_bytes = fast_stream_packet_payload_budget,
        ...
        if (fragments.empty() || fast_stream_new_bytes_sent == 0) {
            break;
        }
        ...
        if (!packet_number.has_value()) {
            for (const auto &fragment : fragments) {
                selected_stream.restore_send_fragment(fragment);
            }
            connection_flow_control_.highest_sent -= fast_stream_new_bytes_sent;
            break;
        }
        ...
        auto app_limited = congestion_controller_.would_underutilize_congestion_window(
        ...
        if (send_profile_enabled()) {
            ...
            profile.stream_bytes += fast_stream_new_bytes_sent;
...
DatagramBuffer QuicConnection::flush_outbound_datagram(QuicCoreTimePoint now,
    ...
    bool has_pto_probe_burst =
        remaining_pto_probe_datagrams_ > 0 &&
        (initial_probe_pending || handshake_probe_pending || application_probe_pending);
    bool has_pto_probe_credit = remaining_pto_probe_datagrams_ > 0;
    bool pto_probe_datagram_requested = has_pto_probe_burst;
    auto preserve_pto_probe_packets = has_pto_probe_burst && remaining_pto_probe_datagrams_ > 1;
    ...
    const auto track_pending_packets_from_datagram =
        ...
        return track_pending_packets(
            [&](const auto &pending) {
                ...
                return std::optional<std::size_t>{
                    tracked_datagram.packet_metadata[pending.packet_index].length,
                };
            },
            tracked_datagram.bytes.size());
    };
    ...
    const auto serialize_candidate_datagram_with_metadata =
        ...
        const auto serialize_datagram = [&](const SerializeProtectionContext &protection_context)
            ...
            if (appended_one_rtt_fragment_packet != nullptr) {
                ...
                const auto appended = append_protected_one_rtt_packet_to_datagram(
                    encoded.value().bytes, *appended_one_rtt_fragment_packet, protection_context);
        ...
        const auto &candidate_serialize_context = context.value();
        ...
        auto serialized_datagram = serialize_datagram(candidate_serialize_context);
        if (!serialized_datagram.has_value()) {
            return serialized_datagram;
        }

        if (serialized_datagram.value().bytes.size() >= kMinimumInitialDatagramSize) {
        ...
        for (std::size_t packet_index = 0; packet_index < datagram_packets->size();
            ...
            const auto padding_deficit =
                kMinimumInitialDatagramSize - serialized_datagram.value().bytes.size();
            ...
            const auto serialize_padded_initial =
                ...
                return serialize_datagram(candidate_serialize_context);
            };

            auto best_oversized_datagram = std::optional<SerializedProtectedDatagram>{};
            const auto remember_candidate = [&](SerializedProtectedDatagram datagram_candidate) {
                const auto candidate_size = datagram_candidate.bytes.size();
                if (candidate_size < kMinimumInitialDatagramSize) {
                    return false;
                }
                if (candidate_size <= max_outbound_datagram_size) {
                    best_oversized_datagram.reset();
                    best_oversized_datagram = std::move(datagram_candidate);
                    return true;
                }
    ...
    const auto commit_serialized_datagram =
...

        {
            ...
            if (pto_probe_datagram_requested) {
            ...
            if (qlog_session_ != nullptr) {
                ...
                for (std::size_t index = 0; index < datagram_packets.size(); ++index) {
                    ...
                    for (auto *packet_space :
                        ...
                        sent->qlog_pto_probe = pto_probe_datagram_requested;
    ...
    const auto finalize_datagram =
        ...
        auto serialized_datagram = serialize_candidate_datagram_with_metadata(datagram_packets);
        if (!serialized_datagram.has_value()) {
    ...
    if ((can_send_one_rtt_packets || use_zero_rtt_packet_protection) &&
        ...
        const auto maybe_queue_client_ack_only_receive_keepalive_challenge = [&]() {
            ...
            if (!((config_.role == EndpointRole::client) & (status_ == HandshakeStatus::connected) &
        ...
        const auto try_send_simple_application_ack_only = [&]() -> std::optional<DatagramBuffer> {
            if (!can_try_simple_application_ack_only(SimpleApplicationAckOnlyEligibility{
                    ...
                    .pto_probe_credit_pending = has_pto_probe_credit,
        ...
        const auto take_stream_fragments = [&](auto &connection_flow, auto &streams,
            ...
            const auto note_selected_payload_bytes = [&](std::size_t first_fragment_index) {
                for (std::size_t index = first_fragment_index; index < output_fragments.size();
                     ++index) {
                    selected_payload_bytes += output_fragments[index].bytes.size();
                }
            };
            const auto visit_round_robin = [&](auto &&visit) {
                const auto visit_stream_range = [&](auto begin, auto end) {
                    for (auto it = begin; it != end; ++it) {
                        if (!visit(it)) {
                            return false;
                        }
                ...
                if (!last_stream_id.has_value()) {
                    static_cast<void>(visit_stream_range(streams.begin(), streams.end()));
                    return;
                }
                ...
                if (!visit_stream_range(start, streams.end())) {
                    return;
                }
                static_cast<void>(visit_stream_range(streams.begin(), start));
            };
            ...
            if (can_try_single_fresh_stream_fast_path && selected_fresh_stream_count == 1) {
                ...
                visit_round_robin([&](const auto it) {
                    ...
                    if (stream_fin_sendable(candidate_stream)) {
                        found_priority_work = true;
                        return false;
                    }
                    if (candidate_stream.sendable_bytes() != 0) {
                        selected = it;
                        return false;
                    }
                    return true;
                ...
                if (!found_priority_work && selected != streams.end()) {
                    auto &round_robin_stream = selected->second;
                    const auto stream_id = selected->first;
                    auto round_robin_previous_fresh_bytes =
                        fresh_sendable_bytes_for_cache(round_robin_stream);
                    auto round_robin_previous_had_lost_data =
                        round_robin_stream.reset_state == StreamControlFrameState::none &&
                        round_robin_stream.send_buffer.has_lost_data();
                    auto round_robin_highest_sent_before =
                        round_robin_stream.flow_control.highest_sent;
                    const auto packet_share =
                        std::min(remaining_wire_bytes,
                                 max_stream_frame_payload_for_wire_budget(
                                     stream_id, round_robin_stream.flow_control.highest_sent,
                    ...
                    const auto fragment_count_before = output_fragments.size();
                    round_robin_stream.append_send_fragments(
                        StreamSendBudget{
                        ...
                        output_fragments);
                    auto round_robin_new_bytes_sent = round_robin_stream.flow_control.highest_sent -
                                                      round_robin_highest_sent_before;
                    connection_flow.highest_sent += round_robin_new_bytes_sent;
                    if (output_fragments.size() != fragment_count_before) {
                        last_stream_id = stream_id;
                        note_selected_payload_bytes(fragment_count_before);
                    }
                    note_stream_send_state_changed(round_robin_previous_fresh_bytes,
                                                   round_robin_previous_had_lost_data,
                                                   round_robin_stream);
                    return selected_payload_bytes;
                }
            }
            ...
            const auto append_selected_stream_fragments =
                ...
                const auto stream_id = selected->first;
                auto &fragment_stream = selected->second;

                auto fragment_stream_previous_fresh_bytes =
                    fresh_sendable_bytes_for_cache(fragment_stream);
                auto fragment_stream_previous_had_lost_data =
                    fragment_stream.reset_state == StreamControlFrameState::none &&
                    fragment_stream.send_buffer.has_lost_data();
                auto fragment_stream_highest_sent_before =
                    fragment_stream.flow_control.highest_sent;
                auto packet_share =
                    loss_phase_for_stream
                        ? std::min(remaining_wire_bytes,
                                   max_stream_frame_payload_for_wire_budget(
                                       stream_id,
                                       fragment_stream.next_send_offset_for_budget(false),
                                       wire_share))
                        : std::min(remaining_wire_bytes,
                                   max_stream_frame_payload_for_wire_budget(
                                       stream_id, fragment_stream.flow_control.highest_sent,
                                       wire_share));
                const auto fin_sendable = stream_fin_sendable(fragment_stream);
                if (packet_share == 0) {
                    if (fin_only_stream_frame_cannot_fit(
                            fin_sendable, fragment_stream.send_final_size.has_value())) {
                        return 0;
                    }
                    const auto fin_only_wire_size = stream_frame_header_wire_size(
                        stream_id, *fragment_stream.send_final_size, 0);
                    if (fin_only_wire_size > remaining_wire_bytes) {
                        return 0;
                    }
                }
                const auto fragment_count_before = output_fragments.size();
                fragment_stream.append_send_fragments(
                    StreamSendBudget{
                        .packet_bytes = packet_share,
                        .new_bytes = new_byte_share,
                    ...
                    output_fragments);
                auto fragment_stream_new_bytes_sent =
                    fragment_stream.flow_control.highest_sent - fragment_stream_highest_sent_before;
                connection_flow.highest_sent += fragment_stream_new_bytes_sent;
                remaining_connection_credit -= std::min<std::uint64_t>(
                    remaining_connection_credit, fragment_stream_new_bytes_sent);
                std::size_t selected_wire_bytes = 0;
                for (std::size_t index = fragment_count_before; index < output_fragments.size();
                     ++index) {
                    auto &fragment = output_fragments[index];
                ...
                const bool emitted_fragment = output_fragments.size() != fragment_count_before;
                if (emitted_fragment) {
                    last_stream_id = stream_id;
                    note_selected_payload_bytes(fragment_count_before);
                }
                note_stream_send_state_changed(fragment_stream_previous_fresh_bytes,
                                               fragment_stream_previous_had_lost_data,
                                               fragment_stream);
            ...
            for (;;) {
                ...
                visit_round_robin([&](const auto it) {
                    ...
                    const auto fin_sendable = stream_fin_sendable(candidate_stream);
                    const auto active =
                        loss_phase ? candidate_stream.send_buffer.has_lost_data() || fin_sendable
                                   : (candidate_stream.sendable_bytes() != 0) || fin_sendable;
                    if (active) {
        ...
        const auto serialize_application_candidate_from_frames =
            ...
            // Application CONNECTION_CLOSE frames force the 1-RTT serialization path below.
            //= https://www.rfc-editor.org/rfc/rfc9000#section-10.2.3
            // # After the handshake is confirmed (see Section 4.1.2 of
            // # [QUIC-TLS]), an endpoint MUST send any CONNECTION_CLOSE frames
            // # in a 1-RTT packet.
        ...
        const auto pad_path_validation_candidate_to_minimum_datagram =
            [&](CodecResult<SerializedProtectedDatagram> &serialized_candidate,
                const PendingPathValidationFrames &path_validation_frames,
                std::span<const Frame> candidate_frames,
                std::span<const StreamFrameSendFragment> stream_fragments,
                bool has_application_close, std::uint64_t candidate_packet_number,
                bool write_key_phase, std::size_t datagram_size_limit) -> bool {
            if (!serialized_candidate.has_value()) {
                return true;
            }
            ...
            if (datagram_size_limit < kMinimumInitialDatagramSize ||
                serialized_candidate.value().bytes.size() >= kMinimumInitialDatagramSize) {
                //= https://www.rfc-editor.org/rfc/rfc9000#section-8.2.1
            ...
            std::size_t padding_length =
                kMinimumInitialDatagramSize - serialized_candidate.value().bytes.size();
            if (!maybe_add_pmtu_probe_padding(padding_length, frames_with_padding,
                                              padding_length)) {
                return true;
            }
            return retry_padded_pmtu_probe_serialization(
                serialized_candidate, frames_with_padding, kMinimumInitialDatagramSize,
                padding_length, [&] {
                    return serialize_application_candidate_from_frames(
        ...
        auto restore_application_fragment = [&](const StreamFrameSendFragment &fragment) {
            ...
            auto &fragment_stream = streams_.at(fragment.stream_id);
            const auto fragment_previous_fresh_bytes =
                fresh_sendable_bytes_for_cache(fragment_stream);
            const auto fragment_previous_had_lost_data =
                fragment_stream.reset_state == StreamControlFrameState::none &&
                fragment_stream.send_buffer.has_lost_data();
            fragment_stream.restore_send_fragment(fragment);
            note_stream_send_state_changed(fragment_previous_fresh_bytes,
                                           fragment_previous_had_lost_data, fragment_stream);
        };
        ...
        if (should_send_application_probe_first()) {
            ...
            const auto make_probe_stream_fragments = [&]() {
                auto rebuilt_probe_fragments = probe_packet.stream_fragments;
                if (rebuilt_probe_fragments.empty() && packet_has_stream_frames(probe_packet)) {
                    rebuilt_probe_fragments.reserve(packet_stream_frame_count(probe_packet));
                    for_each_stream_frame_metadata(
                        probe_packet, [&](const StreamFrameSendMetadata &metadata) {
                            const auto probe_stream_it = streams_.find(metadata.stream_id);
                            if (probe_stream_it == streams_.end()) {
                                return;
                            }
                            const auto bytes = probe_stream_it->second.send_buffer.bytes_for_range(
                                metadata.offset, metadata.length);
                            if (!bytes.has_value()) {
                                return;
                            }
                            auto &fragment =
                                rebuilt_probe_fragments.emplace_back(StreamFrameSendFragment{
                ...
                for (auto &fragment : rebuilt_probe_fragments) {
                    fragment.consumes_flow_control = false;
                }
                return rebuilt_probe_fragments;
            };
            auto restore_probe_fragment = [&](const StreamFrameSendFragment &fragment) {
                const auto probe_stream_it = streams_.find(fragment.stream_id);
                if (probe_stream_it == streams_.end()) {
                    return;
                }

                const auto fragment_previous_fresh_bytes =
                    fresh_sendable_bytes_for_cache(probe_stream_it->second);
                const auto fragment_previous_had_lost_data =
                    probe_stream_it->second.reset_state == StreamControlFrameState::none &&
                    probe_stream_it->second.send_buffer.has_lost_data();
                probe_stream_it->second.mark_send_fragment_lost(fragment);
                note_stream_send_state_changed(fragment_previous_fresh_bytes,
                                               fragment_previous_had_lost_data,
                                               probe_stream_it->second);
            };
            auto mark_probe_fragments_sent =
                [&](std::span<const StreamFrameSendFragment> probe_fragments) {
                    for (const auto &fragment : probe_fragments) {
                        const auto probe_stream_it = streams_.find(fragment.stream_id);
                        if (probe_stream_it == streams_.end()) {
                            continue;
                        }
                        ...
                        const auto fragment_previous_had_lost_data =
                            probe_stream_it->second.reset_state == StreamControlFrameState::none &&
                            probe_stream_it->second.send_buffer.has_lost_data();
                        probe_stream_it->second.mark_send_fragment_sent(fragment);
                        note_stream_send_state_changed(fragment_previous_fresh_bytes,
                                                       fragment_previous_had_lost_data,
                                                       probe_stream_it->second);
                    }
            ...
            if (packet_trace_matches_connection(config_.source_connection_id)) {
                std::cerr << "quic-packet-trace probe-send scid="
                          << format_connection_id_hex(config_.source_connection_id)
                          << " pn=" << *probe_packet_number
                          << " ack=" << static_cast<int>(probe_ack_frame.has_value())
                          << " ack_ranges="
                          << (probe_ack_frame.has_value() ? format_ack_ranges(*probe_ack_frame)
                                                          : std::string{"-"})
                          << " force_ack=" << static_cast<int>(probe_packet.force_ack)
                          << " ping=" << static_cast<int>(include_ping)
                          << " stream=" << probe_stream_fragments.size()
                          << " max_data=" << optional_frame_trace_value(probe_max_data_frame)
                          << " max_stream_data=" << probe_max_stream_data_frames.size()
                          << " path=" << format_optional_path_id(selected_send_path_id)
                          << " bytes=" << datagram.value().bytes.size() << '\n';
            }
                ...
                maybe_queue_ack_only_path_validation_packet(ack_only_path_validation_frames, [&] {
                    ...
                    queue_tracked_packet(
                        ...
                        SentPacketRecord{
                            ...
                            .largest_received_packet_number_acked =
                                ack_only_probing_response_only
                                    ? std::nullopt
            ...
            bool application_probing_response_only =
            ...
            const auto pto_burst_requires_ack_eliciting_application_packet = [&]() {
                if (!has_pto_probe_credit || use_zero_rtt_packet_protection ||
                    send_application_close_only) {
                    return false;
                }
                ...
                return application_candidate_crypto_ranges.empty() && !include_handshake_done &&
                       !application_max_data_frame.has_value() && new_token_frames.empty() &&
                       ...
                       max_stream_data_frames.empty() && max_streams_frames.empty() &&
                       streams_blocked_frames.empty() && reset_stream_frames.empty() &&
                       stop_sending_frames.empty() && !data_blocked_frame.has_value() &&
                       stream_data_blocked_frames.empty() && !selected_datagram_frame.has_value() &&
                       stream_fragments.empty();
            }();
            pto_probe_datagram_requested |= pto_burst_requires_ack_eliciting_application_packet;
            ...
            ApplicationCandidateFrameBuilder application_candidate_frames{
                ApplicationCandidateFrameBuilder::Args{
                    ...
                    .include_ping = pto_burst_requires_ack_eliciting_application_packet,
                }};
            ...
            const auto application_candidate_is_ack_eliciting = [&]() {
                if (pto_burst_requires_ack_eliciting_application_packet) {
                    return true;
                }
            ...
            const auto split_small_terminal_stream_fin_candidate = [&]() -> bool {
                ...
                auto &terminal_fragment = stream_fragments.back();
                const auto terminal_stream_id = terminal_fragment.stream_id;
                terminal_fragment.fin = false;
                terminal_fragment.prime_stream_frame_header_cache();
                mark_stream_terminal_fin_pending(streams_, terminal_stream_id);
            ...
            if (packet_trace_matches_connection(config_.source_connection_id)) {
                ...
                std::cerr << "quic-packet-trace send scid="
                          ...
                          << (selected_ack_frame.has_value()
                                  ? format_ack_ranges(*selected_ack_frame)
                                  : std::string{"-"})
                          ...
                          << " ping="
                          << static_cast<int>(pto_burst_requires_ack_eliciting_application_packet)
            ...
            if (ack_eliciting) {
                ...
                if (can_queue_simple_stream_packet) {
                    ...
                    SentPacketRecord application_sent_record{
                        ...
                        .stream_data_blocked_frames = stream_data_blocked_frames,
                        .stream_fragments = {},
                        .has_ping = pto_burst_requires_ack_eliciting_application_packet,

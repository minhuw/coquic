...
class TlsAdapter::Impl {
  ...
  private:
    ...
    bool configure_context(SSL_CTX *ctx) {
        ...
        if (tls13_cipher_suites_failed(ctx, config_.allowed_tls_cipher_suites)) {
            set_invalid_protection_state();
            return false;
        }
        ...
        if (config_.role == EndpointRole::server && configure_server_resumption_context(ctx)) {
            set_invalid_protection_state();
            return false;
        }
        ...
        if (config_.accept_zero_rtt) {
            SSL_CTX_set_max_early_data(ctx, kQuicMaxEarlyData);
            SSL_CTX_set_recv_max_early_data(ctx, kQuicMaxEarlyData);
        }

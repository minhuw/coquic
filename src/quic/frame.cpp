...
CodecResult<std::vector<std::byte>> read_length_prefixed_bytes(BufferReader &reader) {
    ...
    if (!data.has_value()) {
        return CodecResult<std::vector<std::byte>>::failure(data.error().code, data.error().offset);
    }
...
CodecResult<std::vector<std::byte>> serialize_ack_frame(const AckFrame &frame) {
    ...
    if (const auto error = append_varint(writer, frame.additional_ranges.size())) {
        return failure_result(error->code, error->offset);
    }
...
CodecResult<StreamFrame> decode_stream_frame(BufferReader &reader, std::uint64_t frame_type,
    ...
    if (frame.has_length) {
        ...
        if (!bytes.has_value()) {
            return CodecResult<StreamFrame>::failure(bytes.error().code, bytes.error().offset);
        }

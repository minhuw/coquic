...
CodecResult<ReceivedProtectedPacketDecodeResult> deserialize_received_long_header_packet(
    ...
    // Rebuilding the plaintext header always patches the protected length field to a smaller value.
...
CodecResult<std::size_t> append_protected_long_header_packet_to_datagram(
    ...
    static_cast<void>(writer.write_byte(static_cast<std::byte>(
        0x80u | 0x40u | ((encoded_long_header_type(packet_type, version) & 0x03u) << 4) |
        ((packet_number.packet_number_length - 1) & 0x03u))));
    static_cast<void>(write_u32_be(writer, version));
    static_cast<void>(writer.write_byte(static_cast<std::byte>(destination_connection_id.size())));
    static_cast<void>(writer.write_bytes(destination_connection_id));
    static_cast<void>(writer.write_byte(static_cast<std::byte>(source_connection_id.size())));
    static_cast<void>(writer.write_bytes(source_connection_id));
    if (packet_type == LongHeaderPacketType::initial) {
        writer.write_varint_unchecked(token.size());
        static_cast<void>(writer.write_bytes(token));
    }
    writer.write_varint_unchecked(payload_length);
    static_cast<void>(append_packet_number(writer, packet_number));

    auto payload_bytes = packet_bytes.subspan(header_end, plaintext_payload_size);
    std::size_t payload_offset = 0;
    for (const auto &frame : frames) {
        payload_offset +=
            serialize_frame_into(payload_bytes.subspan(payload_offset), frame).value();
    }
...
CodecResult<std::size_t> packet_stream_payload_wire_size(const OneRttPacketLike &packet,
    ...
    if constexpr (requires { packet.stream_frame_views; }) {
        for (std::size_t stream_index = 0; stream_index < packet.stream_frame_views.size();
            ...
            const auto payload_size = stream_view.end - stream_view.begin;
            if (payload_size != 0) {
                if (!stream_view.storage || stream_view.end > stream_view.storage->size()) {
                    return CodecResult<std::size_t>::failure(CodecErrorCode::invalid_varint,
                                                             frame_index_base + stream_index);
                }
            }
            if (stream_view.stream_id > kMaxVarInt ||
                stream_view.offset > kMaxVarInt - payload_size) {
                return CodecResult<std::size_t>::failure(CodecErrorCode::invalid_varint,
                                                         frame_index_base + stream_index);
            }
...
append_protected_one_rtt_packet_to_datagram_impl(DatagramBuffer &datagram,
    ...
    static_cast<void>(header_writer.write_byte(make_short_header_first_byte(
        packet.spin_bit, packet.key_phase, packet.packet_number_length)));
    static_cast<void>(header_writer.write_bytes(packet.destination_connection_id));
    static_cast<void>(append_packet_number(
    ...
    if constexpr (requires { packet.stream_fragments; }) {
        ...
        // Two or more stream fragments always contribute at least 8 bytes of STREAM frame
        // encoding, so short-header sample padding (0-3 bytes) never changes the payload size.
        ...
        if (can_chunk_seal_stream_fragments) {
            ...
            for (const auto &frame : packet.frames) {
                plaintext_offset +=
                    serialize_frame_into(payload_bytes.subspan(plaintext_offset), frame).value();
                ++frame_index;
            }
    ...
    for (const auto &frame : packet.frames) {
        payload_written +=
            serialize_frame_into(payload_bytes.subspan(payload_written), frame).value();
        ++frame_index;
    }

    if constexpr (requires { packet.stream_frame_views; }) {
        for (const auto &stream_view : packet.stream_frame_views) {
            payload_written += serialize_stream_frame_view_into_span(
...
COQUIC_NO_PROFILE bool protected_codec_internal_coverage_for_tests() {
    ...
    {
        const ReceivedFrame ack = AckFrame{
            .largest_acknowledged = 0,
            .first_ack_range = 0,
        };
...
COQUIC_NO_PROFILE bool protected_codec_packet_path_coverage_for_tests() {
...

    {
        ...
        check("packet_stream_payload_wire_size rejects stream frame views that overrun storage",
              codec_failure_offset(packet_stream_payload_wire_size(
                                       ProtectedOneRttPacketView{
                                           .destination_connection_id = {},
                                           .packet_number_length = 2,

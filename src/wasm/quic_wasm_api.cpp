...
std::optional<TlsIdentity> generate_demo_server_identity() {
    ...
    auto generated_private_key_pem = memory_bio_string(key_bio.get());
    if (!certificate_pem.has_value() || !generated_private_key_pem.has_value()) {
        return std::nullopt;
    }
    return TlsIdentity{
        .certificate_pem = std::move(*certificate_pem),
        .private_key_pem = std::move(*generated_private_key_pem),
    };
}

...
check_secret() {
  ...
  mode="$(stat -c '%a' -- "$path")"
  if [[ "${2:-}" == config ]]; then
    [[ "$mode" == 600 || "$mode" == 400 ]] || fail 'Steward configuration must have mode 0600 or 0400'
    expected_uid="$uid"
  fi
  (( (8#$mode & 077) == 0 )) || fail 'Compose secret permissions are unsafe'
...
check_secret /run/secrets/live-write-token

...
for argument in "$@"; do
  case "$argument" in
    --images|--isolation|--planner|--production-compose) mode="${argument#--}" ;;
...
expected = [
    ...
    ("pre-commit.txt", ["env", "COQUIC_CLANG_TIDY_IN_NIX=1", "COQUIC_CLANG_TIDY_JOBS=2", "bash", "-euc", 'exec pre-commit run --all-files --config "$COQUIC_PRE_COMMIT_CONFIG"']),

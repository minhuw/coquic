...
def signal_item_to_row(
    ...
    *,
    ...
    return SignalItemRow(
        ...
        location_json=_dump_json(item.location),
        links_json=_dump_list(item.links),
        payload_json=_dump_json(item.payload) or "{}",
...
def row_to_signal_item(row: SignalItemRow, *, path_codec: PathCodec) -> SignalItem:
    return SignalItem(
        ...
        location=_loads_optional_dict(row.location_json),
        links=_loads_list(row.links_json),

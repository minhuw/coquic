...
TEST(QuicCoreTest, DrainOutboundDatagramReplaysDeferredProtectedPacketsBeforeFlush) {
    ...
    connection.deferred_protected_packets_.push_back(coquic::quic::DeferredProtectedDatagram(
        deferred_packet.value(), /*id=*/0, std::nullopt,
        coquic::quic::QuicEcnCodepoint::unavailable, coquic::quic::test::test_time(1)));

    auto datagram = connection.drain_outbound_datagram(coquic::quic::test::test_time(29));
    ...
    for (const auto &frame : application->frames) {
        if (const auto *ack = std::get_if<coquic::quic::AckFrame>(&frame)) {
            ...
            EXPECT_EQ(ack->ack_delay, 28000u);
        }
    }
    EXPECT_TRUE(saw_ack);
}

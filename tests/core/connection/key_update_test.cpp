...
TEST(QuicCoreTest, LocalKeyUpdateCompletesWhenUpdatedPhasePacketIsAcknowledged) {
    auto connection = make_connected_client_connection();
    ...
    auto current_ack_result = connection.process_inbound_ack(
        connection.application_space_,
        coquic::quic::AckFrame{
            .largest_acknowledged = current_phase_packet_number,
            .first_ack_range = 0,
        },
        coquic::quic::test::test_time(2), connection.config_.transport.ack_delay_exponent,
        connection.config_.transport.max_ack_delay, false);
    ASSERT_TRUE(current_ack_result.has_value());
    ...
    auto updated_ack_result = connection.process_inbound_ack(
        ...
        connection.config_.transport.max_ack_delay, false);
    ASSERT_TRUE(updated_ack_result.has_value());

    EXPECT_FALSE(connection.local_key_update_initiated_);
    EXPECT_FALSE(connection.local_key_update_requested_);
    EXPECT_FALSE(connection.current_write_phase_first_packet_number_.has_value());
    EXPECT_EQ(connection.application_write_key_phase_, !original_write_key_phase);
}
...
TEST(QuicCoreTest, ApplicationReceivePrecomputesNextReadKey) {
    ...
    ASSERT_TRUE(current_phase_packet.has_value());

    connection.process_inbound_datagram(current_phase_packet.value(),
                                        coquic::quic::test::test_time(1));
    ...
    EXPECT_EQ(optional_ref_or_terminate(connection.next_application_read_secret_).secret,
              expected_next_read_secret.value().secret);
    EXPECT_EQ(connection.next_application_read_key_phase_, !connection.application_read_key_phase_);
}
...
TEST(QuicCoreTest, OldKeyAckOfCurrentKeyUpdatePacketClosesWithKeyUpdateError) {
    auto connection = make_connected_client_connection();
    ASSERT_TRUE(connection.application_space_.read_secret.has_value());
    ASSERT_TRUE(connection.application_space_.write_secret.has_value());
    ...
    auto old_key_ack = coquic::quic::serialize_protected_datagram(
        ...
        coquic::quic::SerializeProtectionContext{
            ...
            .client_initial_destination_connection_id =
                connection.client_initial_destination_connection_id(),
            .one_rtt_secret = old_read_secret,
            .one_rtt_key_phase = old_read_key_phase,
        });
    ASSERT_TRUE(old_key_ack.has_value());

    connection.process_inbound_datagram(old_key_ack.value(), coquic::quic::test::test_time(4));
    ...
    EXPECT_EQ(optional_ref_or_terminate(connection.pending_transport_close_).error_code,
              static_cast<std::uint64_t>(coquic::quic::QuicTransportErrorCode::key_update_error));
}
...
TEST(QuicCoreTest, ProactiveKeyUpdateRequestsBeforeAesGcmConfidentialityLimit) {
    auto connection = make_connected_client_connection();
    ASSERT_TRUE(connection.application_space_.write_secret.has_value());
    ASSERT_EQ(optional_ref_or_terminate(connection.application_space_.write_secret).cipher_suite,
              coquic::quic::CipherSuite::tls_aes_128_gcm_sha256);
    constexpr std::uint64_t threshold = std::uint64_t{1} << 22u;
    ASSERT_TRUE(threshold > 0u);

    connection.current_application_write_key_encrypted_packets_ = threshold - 1;

    EXPECT_TRUE(connection.note_aead_encryption_attempt(1, coquic::quic::test::test_time(1)));

    EXPECT_TRUE(connection.local_key_update_requested_);
    EXPECT_FALSE(connection.local_key_update_initiated_);
    ASSERT_TRUE(connection.current_write_phase_first_packet_number_.has_value());
    EXPECT_EQ(optional_value_or_terminate(connection.current_write_phase_first_packet_number_),
              connection.application_space_.next_send_packet_number);
    EXPECT_FALSE(connection.has_failed());
}
...
TEST(QuicCoreTest, CompletedLocalKeyUpdateCanBeRequestedAgain) {
    auto connection = make_connected_client_connection();
    const auto original_write_key_phase = connection.application_write_key_phase_;
    connection.request_key_update();

    ASSERT_TRUE(connection.queue_stream_send(0, bytes_from_ints({0x61}), false).has_value());
    auto first_current_phase_datagram =
        connection.drain_outbound_datagram(coquic::quic::test::test_time(1));
    ASSERT_FALSE(first_current_phase_datagram.empty());
    const auto first_current_phase_packet_number =
        last_tracked_packet(connection.application_space_).packet_number;
    ...
    ASSERT_TRUE(
        connection
            .process_inbound_ack(connection.application_space_,
                                 coquic::quic::AckFrame{
                                     .largest_acknowledged = first_current_phase_packet_number,
                                     .first_ack_range = 0,
                                 },
                                 coquic::quic::test::test_time(2),
                                 connection.config_.transport.ack_delay_exponent,
    ...
    ASSERT_TRUE(connection.queue_stream_send(8, bytes_from_ints({0x63}), false).has_value());
    ...
    auto second_updated_phase_datagram =
        connection.drain_outbound_datagram(coquic::quic::test::test_time(7));
    ASSERT_FALSE(second_updated_phase_datagram.empty());

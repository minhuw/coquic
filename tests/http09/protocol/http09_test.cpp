...
TEST(QuicHttp09Test, RejectsMixedAuthoritiesInSingleClientRun) {
    const auto parsed =
        coquic::http09::parse_http09_requests_env("https://a.test/x https://b.test/y");
...
TEST(QuicHttp09Test, RejectsUnsafeRequestTargetsDuringRequestEnvParse) {
    EXPECT_FALSE(
        coquic::http09::parse_http09_requests_env("https://example.test/../secret").has_value());
    EXPECT_FALSE(
        coquic::http09::parse_http09_requests_env("https://example.test/a/../b").has_value());
    EXPECT_FALSE(coquic::http09::parse_http09_requests_env("https://example.test/a?b").has_value());
    EXPECT_FALSE(coquic::http09::parse_http09_requests_env("https://example.test/a#b").has_value());
}

TEST(QuicHttp09Test, RejectsEmptyRequestsEnvAndUrlsWithoutAuthority) {
    EXPECT_FALSE(coquic::http09::parse_http09_requests_env("").has_value());
    EXPECT_FALSE(coquic::http09::parse_http09_requests_env("https://").has_value());
    EXPECT_FALSE(coquic::http09::parse_http09_requests_env("https:///a").has_value());
}
...
TEST(QuicHttp09Test, NormalizesAuthorityWithoutPathToSlashAndRejectsWrongScheme) {
    const auto parsed = coquic::http09::parse_http09_requests_env("https://example.test");
    ASSERT_TRUE(parsed.has_value());
    ASSERT_EQ(parsed.value().size(), 1u);
    EXPECT_EQ(parsed.value()[0].authority, "example.test");
    EXPECT_EQ(parsed.value()[0].request_target, "/");

    EXPECT_FALSE(coquic::http09::parse_http09_requests_env("http://example.test/a").has_value());
}

TEST(QuicHttp09Test, UsesGenerousTransferFlowControlProfileForTransferCase) {
    const auto config = coquic::http09::http09_client_transport_for_testcase(
...
TEST(QuicHttp09Test, MigrationAliasesReuseTransferTransportAndTlsProfiles) {
    ...
    const auto transfer_server = coquic::http09::http09_server_transport_for_testcase(
        coquic::http09::QuicHttp09Testcase::transfer);
    const auto transfer_tls = coquic::http09::http09_tls_cipher_suites_for_testcase(
        coquic::http09::QuicHttp09Testcase::transfer);

    for (const auto testcase :
         {coquic::http09::QuicHttp09Testcase::rebind_port,
          coquic::http09::QuicHttp09Testcase::rebind_addr, coquic::http09::QuicHttp09Testcase::ecn,
          coquic::http09::QuicHttp09Testcase::connectionmigration}) {
        const auto client = coquic::http09::http09_client_transport_for_testcase(testcase);
        ...
        const auto server = coquic::http09::http09_server_transport_for_testcase(testcase);
        EXPECT_EQ(server.initial_max_streams_bidi, transfer_server.initial_max_streams_bidi);

        EXPECT_EQ(coquic::http09::http09_tls_cipher_suites_for_testcase(testcase), transfer_tls);
    }
}
...
TEST(QuicHttp09Test, KeyUpdateReusesTransferTransportAndTlsProfiles) {
    ...
    const auto keyupdate_client = coquic::http09::http09_client_transport_for_testcase(
    ...
    const auto keyupdate_server = coquic::http09::http09_server_transport_for_testcase(
        coquic::http09::QuicHttp09Testcase::keyupdate);
    EXPECT_EQ(keyupdate_server.initial_max_streams_bidi, transfer_server.initial_max_streams_bidi);

    EXPECT_EQ(coquic::http09::http09_tls_cipher_suites_for_testcase(
                  coquic::http09::QuicHttp09Testcase::keyupdate),
              transfer_tls);
}

TEST(QuicHttp09Test, UsesExtendedIdleTimeoutProfileForInteropCases) {
    const auto transfer_client = coquic::http09::http09_client_transport_for_testcase(
        coquic::http09::QuicHttp09Testcase::transfer);
...
TEST(QuicHttp09Test, UsesExpandedServerStreamProfileForResumptionAndZeroRttCases) {
    const auto resumption = coquic::http09::http09_server_transport_for_testcase(
        coquic::http09::QuicHttp09Testcase::resumption);
    EXPECT_EQ(resumption.initial_max_streams_bidi, 64u);

    const auto zero_rtt = coquic::http09::http09_server_transport_for_testcase(
...
TEST(QuicHttp09Test, UsesStableZeroRttContextAcrossDifferentGetRequestSets) {
    ...
    const auto transfer = coquic::http09::parse_http09_requests_env(
        "https://example.test/warmup-only.txt https://example.test/final.txt");
    ASSERT_TRUE(transfer.has_value());

    const auto warmup_context = coquic::http09::http09_zero_rtt_application_context(warmup.value());
    const auto transfer_context =
        coquic::http09::http09_zero_rtt_application_context(transfer.value());
...
TEST(QuicHttp09Test, EmptyRequestSetProducesEmptyZeroRttContext) {
    const auto context = coquic::http09::http09_zero_rtt_application_context(
        std::span<const coquic::http09::QuicHttp09Request>{});
    EXPECT_TRUE(context.empty());
}

TEST(QuicHttp09Test, ResolvesRequestTargetUnderRootWithoutDiscardingRoot) {
    const auto resolved =
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "/a/b.bin");
...
TEST(QuicHttp09Test, RejectsTraversalQueriesAndFragmentsWhenResolvingPath) {
    EXPECT_FALSE(
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "/../secret").has_value());
    EXPECT_FALSE(
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "/a/../b").has_value());
    EXPECT_FALSE(
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "/a?b").has_value());
    EXPECT_FALSE(
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "/a#b").has_value());
...
TEST(QuicHttp09Test, RejectsMalformedTargetsWhenResolvingPath) {
    EXPECT_FALSE(coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "").has_value());
    EXPECT_FALSE(
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "relative").has_value());
    EXPECT_FALSE(
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "//absolute").has_value());
    EXPECT_FALSE(
        coquic::http09::resolve_http09_path_under_root("/tmp/downloads", "/nested/.").has_value());
}

TEST(QuicHttp09Test, RejectsRelativeRootsThatLoseTheirLexicalPrefix) {
    EXPECT_FALSE(coquic::http09::resolve_http09_path_under_root(".", "/payload.bin").has_value());
}

TEST(QuicHttp09Test, RejectsAbsoluteHttpsRequestWithDoubleSlashPath) {
    EXPECT_FALSE(
        coquic::http09::parse_http09_requests_env("https://example.test//absolute").has_value());
}

TEST(QuicHttp09Test, ParsesRequestTargetFromCrLfTerminatedLine) {
    const auto parsed = coquic::http09::parse_http09_request_target(bytes_from_ascii("GET /a\r\n"));
    ASSERT_TRUE(parsed.has_value());
    EXPECT_EQ(parsed.value(), "/a");
}

TEST(QuicHttp09Test, ParsesRequestTargetFromLfTerminatedLine) {
    const auto parsed = coquic::http09::parse_http09_request_target(bytes_from_ascii("GET /b\n"));
    ASSERT_TRUE(parsed.has_value());
...
TEST(QuicHttp09Test, ParsesOnlyFirstRequestLineWhenBufferHasTrailingBytes) {
    const auto parsed =
        coquic::http09::parse_http09_request_target(bytes_from_ascii("GET /c\r\nEXTRA"));
    ASSERT_TRUE(parsed.has_value());
...
TEST(QuicHttp09Test, RejectsEmptyRequestLine) {
    const auto parsed = coquic::http09::parse_http09_request_target(bytes_from_ascii("\n"));
    ASSERT_FALSE(parsed.has_value());
    EXPECT_EQ(parsed.error().code, coquic::quic::CodecErrorCode::http09_parse_error);
}
...
TEST(QuicHttp09Test, RejectsTabCharacterInRequestTarget) {
    const auto parsed =
        coquic::http09::parse_http09_request_target(bytes_from_ascii("GET /\tbad\r\n"));
    ASSERT_FALSE(parsed.has_value());
    EXPECT_EQ(parsed.error().code, coquic::quic::CodecErrorCode::http09_parse_error);
}
...
TEST(QuicHttp09Test, RejectsDeleteCharacterInRequestTarget) {
    const auto parsed =
        coquic::http09::parse_http09_request_target(bytes_from_ascii("GET /\x7f\r\n"));
    ASSERT_FALSE(parsed.has_value());
    EXPECT_EQ(parsed.error().code, coquic::quic::CodecErrorCode::http09_parse_error);
}
...
TEST(QuicHttp09Test, RejectsEmptyAndRelativeRequestTargets) {
    EXPECT_FALSE(
        coquic::http09::parse_http09_request_target(bytes_from_ascii("GET \r\n")).has_value());
    EXPECT_FALSE(coquic::http09::parse_http09_request_target(bytes_from_ascii("GET relative\r\n"))
                     .has_value());
}

...
TEST(QuicHttp3ProtocolTest, EncodesAndDecodesSettingsAndGoawayFrames) {
    ...
    auto control_stream = coquic::http3::serialize_http3_control_stream(settings);
    ASSERT_TRUE(control_stream.has_value());

    auto control_stream_type = coquic::http3::parse_http3_uni_stream_type(control_stream.value());
    ...
    auto decoded_settings = coquic::http3::parse_http3_frame(settings_bytes.value());
    ...
    auto decoded_goaway = coquic::http3::parse_http3_frame(goaway_bytes.value());
...
TEST(QuicHttp3ProtocolTest, RejectsDuplicateSettingsAndBadPseudoHeaderOrdering) {
    ...
    std::array uppercase_fields{
        coquic::http3::Http3Field{":method", "GET"},
        coquic::http3::Http3Field{":scheme", "https"},
        coquic::http3::Http3Field{":path", "/"},
        coquic::http3::Http3Field{"X-Upper", "1"},
    };
    auto uppercase_result = coquic::http3::validate_http3_request_headers(uppercase_fields);
    ASSERT_FALSE(uppercase_result.has_value());
    EXPECT_EQ(uppercase_result.error().code, coquic::http3::Http3ErrorCode::message_error);

    std::array empty_name_fields{
        coquic::http3::Http3Field{":status", "200"},
        coquic::http3::Http3Field{"", "1"},
    };
    auto empty_name_result = coquic::http3::validate_http3_response_headers(empty_name_fields);
...
TEST(QuicHttp3ProtocolTest, RejectsRequestsWithEmptyAuthorityOrHostWhenPresent) {
    const std::array empty_authority_fields{
        coquic::http3::Http3Field{":method", "GET"},
        coquic::http3::Http3Field{":scheme", "https"},
        coquic::http3::Http3Field{":authority", ""},
        coquic::http3::Http3Field{":path", "/"},
        coquic::http3::Http3Field{"host", "example.test"},
    };
    const auto empty_authority =
        coquic::http3::validate_http3_request_headers(empty_authority_fields);
    ASSERT_FALSE(empty_authority.has_value());
    EXPECT_EQ(empty_authority.error().code, coquic::http3::Http3ErrorCode::message_error);
    EXPECT_EQ(empty_authority.error().detail, "empty :authority pseudo header");

    const std::array empty_host_fields{
        coquic::http3::Http3Field{":method", "GET"},
        coquic::http3::Http3Field{":scheme", "https"},
        coquic::http3::Http3Field{":authority", "example.test"},
        coquic::http3::Http3Field{":path", "/"},
        coquic::http3::Http3Field{"host", ""},
    };
    const auto empty_host = coquic::http3::validate_http3_request_headers(empty_host_fields);
    ASSERT_FALSE(empty_host.has_value());
    EXPECT_EQ(empty_host.error().code, coquic::http3::Http3ErrorCode::message_error);
    EXPECT_EQ(empty_host.error().detail, "empty host header");
}
...
TEST(QuicHttp3ProtocolTest, ControlStreamMaxPushIdAllowanceDependsOnReceiverRole) {
    ...
    const auto max_push_id = Http3Frame{Http3MaxPushIdFrame{.push_id = 0}};
    EXPECT_TRUE(coquic::http3::http3_frame_allowed_on_control_stream(Http3ConnectionRole::server,
                                                                     max_push_id));
...
TEST(QuicHttp3ProtocolTest, EncodesAndDecodesDataAndHeadersFrames) {
    ...
    auto decoded_headers = coquic::http3::parse_http3_frame(encoded_headers.value());
...
TEST(QuicHttp3ProtocolTest, RejectsFramesThatCannotBeSerializedAsVarints) {
    ...
    auto invalid_control_stream = coquic::http3::serialize_http3_control_stream(
...
TEST(QuicHttp3ProtocolTest, RejectsMalformedFrameEncodings) {
    ...
    auto truncated_goaway = coquic::http3::parse_http3_frame(bytes_from_ints({0x07, 0x01, 0x40}));
...
TEST(QuicHttp3ProtocolTest, AcceptsValidSettingsRequestsResponsesAndTrailers) {
    ...
    std::array response_fields{
        coquic::http3::Http3Field{":status", "204"},
        coquic::http3::Http3Field{"server", "coquic"},
    };
    auto response = coquic::http3::validate_http3_response_headers(response_fields);
    ...
    std::array trailers_fields{
        coquic::http3::Http3Field{"etag", "abc123"},
    };
    auto trailers = coquic::http3::validate_http3_trailers(trailers_fields);
...
TEST(QuicHttp3ProtocolTest, RejectsInvalidResponseHeadersAndTrailers) {
    ...
    for (const auto &test_case : std::array{
        ...
        auto response = coquic::http3::validate_http3_response_headers(test_case.fields);

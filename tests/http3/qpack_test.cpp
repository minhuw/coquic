...
TEST(QuicHttp3QpackTest, RejectsLiteralFieldsLongerThanSingleByteLengthPrefix) {
    const coquic::quic::Http3Headers long_name_headers = {{
        .name = std::string(256, 'n'),
        .value = "value",
    }};

    const auto encoded_long_name = coquic::quic::encode_http3_field_section(long_name_headers);
    ...
    const coquic::quic::Http3Headers long_value_headers = {{
        .name = "name",
        .value = std::string(256, 'v'),
    }};

    const auto encoded_long_value = coquic::quic::encode_http3_field_section(long_value_headers);
...
TEST(QuicHttp3QpackTest, AcceptsEmptyInstructionsWhenDynamicTableAndBlockedStreamsAreZero) {
    ...
    const auto encoder = coquic::quic::validate_http3_qpack_encoder_stream(empty_bytes, settings);
    ASSERT_TRUE(encoder.has_value());
    EXPECT_TRUE(encoder.value());

    const auto decoder = coquic::quic::validate_http3_qpack_decoder_stream(empty_bytes, settings);
...
TEST(QuicHttp3QpackTest, RejectsMalformedAndTruncatedLiteralFieldSections) {
    for (const auto &[bytes, detail] : std::array{
        ...
        const auto decoded = coquic::quic::decode_http3_field_section(bytes);
        ASSERT_FALSE(decoded.has_value());
        EXPECT_EQ(decoded.error().code, coquic::quic::Http3ErrorCode::qpack_decompression_failed);
...
TEST(QuicHttp3QpackTest, RejectsWrongLiteralPrefixesAndOutOfRangeStaticIndexes) {
    const auto wrong_prefix = coquic::quic::decode_http3_field_section(bytes_from_ints({
        0x00,
        0x01,
        'n',
        0x01,
    ...
    const auto out_of_range = coquic::quic::decode_http3_field_section(bytes_from_ints({
        0x80,
        0x0a,
    }));
...
TEST(QuicHttp3QpackTest, AcceptsNonEmptyEncoderAndDecoderInstructionsWhenSettingsAllowThem) {
    const auto encoder = coquic::quic::validate_http3_qpack_encoder_stream(
        bytes_from_ints({0x3f, 0xe1, 0x1f}),
        coquic::quic::Http3QpackSettings{.max_table_capacity = 32, .blocked_streams = 0});
    ASSERT_TRUE(encoder.has_value());
    EXPECT_TRUE(encoder.value());

    const auto decoder = coquic::quic::validate_http3_qpack_decoder_stream(
        bytes_from_ints({0x80}),
        coquic::quic::Http3QpackSettings{.max_table_capacity = 0, .blocked_streams = 1});
    ASSERT_TRUE(decoder.has_value());
    EXPECT_TRUE(decoder.value());
}
